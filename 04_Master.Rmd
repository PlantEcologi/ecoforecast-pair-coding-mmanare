...
>*Answer 1:*The reason why it saturates linearly with time is possibly because of the possible climatic conditions through out the year which affect the gradual recovery of plants postfire from seeds to mature crops. If the favourable conditions were consistent then seeds would have successfully geminated as per the use of prescribed burns and eventually the crops would have matured and remained healthy and green. 
<br> 
...
>*Answer 2:* Alpha and lambda estimates in the first model are relatively high compared to the second model,how ever the estimate for gamma is high in the second model relative to the first.These being the only estimates that appear in both models.The standard error associated with these estimates is generally lower in the second thus implying more accuracy from this model compared to the first.
<br>
...
>*Answer 3:*Model 2,a very small p-value (4.12e-14) imlplies significantly improved fit over the model 2 after adding the other variables.
...
>*Answer 5:*lowest AIC is always better, hence AIC_full = 6 + 2*fit_negexpMLES$value is a better model.
<br>
...
>*Answer 6:*AIC theory requires that the log-likelihood is maximized. AIC can be computed for models that are not fitted with maximum likelihood, but there is no need to compare their AIC values.
<br>